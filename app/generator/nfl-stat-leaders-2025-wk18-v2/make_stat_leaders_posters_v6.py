...
# EXACT URLS (including your interceptions link)
URLS = {
    "Passing Yards": (
    ...
    "Sacks": (
        ...
        "float1",  # sacks show as 23.0 / 16.5 etc
    ),
    "Tackles": (
...
def safe_int(x) -> Optional[int]:
    ...
    if f is None:
        return None
    # ESPN ints sometimes come as "1.0" -> cast safely
    return int(round(f))
...
def parse_name_team(name_cell: str) -> Tuple[str, str]:
    ...
    Ensures we can print: "Matthew Stafford LAR"
    """
...
def choose_stat_col(df: pd.DataFrame, candidates: List[str]) -> str:
    ...
    # substring (handles "Interceptions INT", "Sacks SACK", etc.)
    for ck in cand_l:
        for i, colk in enumerate(cols_l):
            if ck in colk:
                return cols[i]
...
def topN_from_url(url: str, stat_candidates: List[str], mode: str) -> List[Tuple[int, str, Number]]:
    ...
    for i, rec in enumerate(work.to_dict("records"), start=1):
        ...
        display_name = f"{player} {team}".strip()  # <-- ensures space before team
...
def main():
    ...
    out_off = os.path.join(desktop, f"offense_stat_leaders_{SEASON}_wk18_v6.png")
    out_def = os.path.join(desktop, f"defense_stat_leaders_{SEASON}_wk18_v6.png")
