...
def parse_name_team(name_cell: str) -> Tuple[str, str]:
    """
    Fixes ESPN cases like:
      'Matthew StaffordLAR'  -> ('Matthew Stafford', 'LAR')
      'Matthew Stafford LAR' -> ('Matthew Stafford', 'LAR')
    """
    s = str(name_cell).strip()
    s = re.sub(r"\s+", " ", s)

    # Case 1: last token is team
    parts = s.split(" ")
    if len(parts) >= 2 and re.fullmatch(r"[A-Z]{2,4}", parts[-1]):
        return " ".join(parts[:-1]).strip(), parts[-1].strip()

    # Case 2: team glued to end (no space)
    m = re.search(r"^(.*?)([A-Z]{2,4})$", s)
    if m:
        name = m.group(1).strip()
        team = m.group(2).strip()
        # avoid false positives like "III" etc by requiring name has a space
        if " " in name:
            return name, team
...
def topN_from_url(url: str, stat_candidates: List[str], mode: str) -> List[Tuple[int, str, Number]]:
    ...
    for i, rec in enumerate(work.to_dict("records"), start=1):
        ...
        # FORCE a single space between name and team
...
def draw_poster(out_path: str, title: str, subtitle: str,
    ...
    # BIGGER text so the boxes feel "full"
    title_font = load_font(56, bold=True)
    sub_font = load_font(28, bold=False)
    head_font = load_font(32, bold=True)
    row_font = load_font(26, bold=False)   # was 22
    ...
    for idx, (sec_title, items, mode) in enumerate(sections):
        ...
        y = y0 + 64
        line_h = 33  # tuned so 10 rows fit but fill more vertical space
...
def main():
    ...
    out_off = os.path.join(desktop, f"offense_stat_leaders_{SEASON}_wk18_v7.png")
    out_def = os.path.join(desktop, f"defense_stat_leaders_{SEASON}_wk18_v7.png")
