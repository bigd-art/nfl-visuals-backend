...
def find_stat_table(tables: List[pd.DataFrame], candidates: List[str]) -> Optional[pd.DataFrame]:
    ...
    for t in tables:
        ...
        if len(cand_l.intersection(cols_l)) > 0:
            return t
    return None
...
def choose_stat_col(df: pd.DataFrame, candidates: List[str]) -> str:
    """
    ESPN headers are often like:
      'Sacks SACK', 'Tackles TOT', 'Interceptions INT'
    so we support:
      1) exact match (case-insensitive)
      2) substring match (candidate contained in column name)
    """
    ...
    # 1) exact match
    for ckey in cand_l:
        for i, col_key in enumerate(cols_l):
            if col_key == ckey:
                return cols[i]

    # 2) substring match (handles "Sacks SACK" etc)
    for ckey in cand_l:
        for i, col_key in enumerate(cols_l):
            if ckey in col_key:
                return cols[i]
...
def main():
    ...
    for t in offense_titles:
        url, cand = URLS[t]
        offense_sections.append((t, top10_from_url(url, cand)))
    ...
    for t in defense_titles:
        ...
        defense_sections.append((label, top10_from_url(url, cand)))

    desktop = os.path.join(os.path.expanduser("~"), "Desktop")
    out_off = os.path.join(desktop, f"offense_stat_leaders_{SEASON}_wk18_v5.png")
    out_def = os.path.join(desktop, f"defense_stat_leaders_{SEASON}_wk18_v5.png")
