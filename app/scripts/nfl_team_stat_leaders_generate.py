        ...
        df.columns = [
            " ".join(
                [
                    str(x).strip()
                    for x in tup
                    if str(x).strip() and "Unnamed" not in str(x)
                ]
            ).strip()
...
# ============================================================
# ✅ THESE TWO FUNCTIONS MUST EXIST (router imports them)
# ============================================================

def extract_team_leaders(team_url: str) -> Dict:
    """
    Scrape ESPN team stats page -> return structured leaders data.
    Router expects this function name.
    """
    tables = fetch_tables(team_url)

    # Same indices you debugged:
    # 0 name(pass),1 passing
    # 2 name(rush),3 rushing
    # 4 name(rec), 5 receiving
    # 6 name(def), 7 defense stats
    ...
    # DEFENSE: find the right columns robustly
...
def draw_leaders_grid_poster(team: str, team_url: str, out_dir: Optional[str] = None) -> Tuple[str, str]:
    """
    Create TWO PNGs (offense + defense) and return their file paths.
    Router expects this function name.
    """
    ...
    if out_dir is None:
        out_dir = "/tmp"  # Render-friendly
    os.makedirs(out_dir, exist_ok=True)

    out_off = os.path.join(out_dir, f"team_offense_leaders_{team}.png")
    out_def = os.path.join(out_dir, f"team_defense_leaders_{team}.png")

    draw_team_poster(out_off, "Offensive Team Leaders", subtitle, leaders["offense"])
