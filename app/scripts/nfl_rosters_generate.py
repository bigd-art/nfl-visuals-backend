...
def get_depthchart_groups(
    ...
    # In case the endpoint returns one expanded
    # depth-chart object directly.
    if (
        "positions"
        in data
        and isinstance(
...
def resolve_depth_athlete(
    ...
    # Sometimes the athlete object itself is expanded.
    if (
        athlete.get(
            "displayName"
        )
    ...
    # Sometimes there's a nested athlete reference.
    nested = (
        athlete.get(
            "athlete"
        )
...
def get_team_code_from_args():
    ...
    if len(
        ...
        print(
            "Usage: python3 "
            "nfl_rosters_espn_depthchart_labels.py "
            "<team_code> [year]"
        )
...
def main():
    ...
    output_dir = (
        "single_team_roster_"
        "espn_depthchart_labels_posters"
    )
